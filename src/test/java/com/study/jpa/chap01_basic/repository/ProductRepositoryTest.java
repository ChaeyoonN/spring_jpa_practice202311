...
@Rollback(false) // true가 기본값, false: 롤백을 실행하지 않는다.
class ProductRepositoryTest {
    ...
    void testSave() {
        ...
        Product p5= Product.builder()
                ...
                .build();
        //when
        Product saved = productRepository.save(p5); // 정상적으로 인서트되면 인서트한 객체를 리턴.(비정상:null 리턴)
    ...
    void testModify() {
        ...
        // 조회한 수 setter로 변경하면 자동으로 update문이 나갑니다.
        // 변경 후에 save를 호출하세요. 그럼 update가 나갑니다.
        Optional<Product> product = productRepository.findById(id);
        product.ifPresent(p->{
            p.setName(newName);
            p.setPrice(newPrice);
        ...
        Product p = product.get();assertEquals("짜장면", p.getName());
