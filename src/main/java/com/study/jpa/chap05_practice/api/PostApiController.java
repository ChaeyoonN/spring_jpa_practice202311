...
@RestController // 화면단이 리액트(모든 것을 비동기 방식으로 처리)이므로 레스트컨트롤러
@Slf4j
...
public class PostApiController {
    ...
    @GetMapping
    public ResponseEntity<?> list(PageDTO pageDTO){ // pageDTO: 커멘드 객체
        // (변수명이랑 필드명이 같으면 커맨드객체 필드에 값이 들어가는 거)
...
//        }
    ...
    @Parameters({
            ...
            @Parameter(name = "hashTags", description = "게시물의 해시태그를 작성하세요!", example = "['하하', '호호']", required = false),
    })
    @PostMapping
    ...
    public ResponseEntity<?> update(
            ...
            HttpServletRequest request // 요청 방식에 따른 처리 구분할 때 요긴하다.
    ){
    ...
    // 입력값 검증(Validation)의 결과를 처리해 주는 전역 메서드 (ctrl+alt+m)
